...
import subprocess
import stat
from problems.storage.factory import get_storage_provider
...
class PolygonAPI:
    ...
    def _make_request(self, method_name, params=None, expect_json=True, return_raw=False):
        """
        ...
        Args:
            ...
            return_raw (bool, optional): Whether to return the raw response object. Defaults to False.

        Returns:
            dict or str or Response: The parsed JSON result, raw text, or response object.
        ...
        try:
            response = requests.post(f"{self.API_URL}{method_name}", data=post_params, verify=False)
    ...
    def download_and_extract_package(self, problem_id, package_type='standard'):
        ...
        try:
            ...
            response = requests.post(f"{self.API_URL}problem.package", data=post_params, verify=False)
            response.raise_for_status()
    ...
    def upload_custom_checker_to_azure(self, problem_id, db_problem_id=None):
        """
        Fetches, compiles, and uploads custom checker using the configured StorageProvider.
        """
        ...
        if checker_dir:
            ...
            if not binary_path:
                ...
                try:                    
                    ...
                    storage_provider.upload(blob_name, source_code.encode('utf-8'))
            ...
            logger.info("Uploading custom checker to: %s", blob_name)
            
            try:
                storage_provider.upload(blob_name, binary_data)
    ...
    def migrate_to_azure_blob(self, problem_id, db_problem_id=None, testset='tests'):
        """
        Fetches all test cases and uploads them to configured Storage Provider.
        ...
        logger.info("Deleting existing test cases from Storage")
        prefix = f"test_cases/{problem_id_for_naming}/"
        storage_provider.delete_by_prefix(prefix)
        ...
        for idx, test in enumerate(test_cases, start=1):
            ...
            if input_data and output_data:
                ...
                # Logic used in previous AzureTestcase upload_test_case
                input_blob_name = f"test_cases/{problem_id_for_naming}/{idx:02d}"
                output_blob_name = f"test_cases/{problem_id_for_naming}/{idx:02d}.a"
                
                try: 
                    # Upload input
                    storage_provider.upload(input_blob_name, input_data.encode('utf-8'))
                    
                    # Upload output
                    storage_provider.upload(output_blob_name, output_data.encode('utf-8'))
                    
                    uploaded_count += 1
                except Exception as e:
                    logger.error(f"Error uploading test case #{idx}: {e}")
        ...
        logger.info("Uploaded %d test cases to Storage", uploaded_count)
    ...
    def _get_redis_connection(self):
        """Creates a Redis connection with correct SSL and timeout settings."""
