            ...
            try:
                with transaction.atomic():
                    ...
                    if migrate_to_azure:
                        ...
                        # Use the database problem ID for naming
                        problem_id = problem_obj.id if problem_obj else None
                        logger.info('Storage migration params: problem_id=%s', problem_id)
                        ...
                        if custom_checker_info:
                            logger.info('Custom checker detected before Storage migration: %s', custom_checker_info)
                            context['info'] = f"Custom checker '{custom_checker_info['name']}' detected. Will be compiled and uploaded."
                        
                        logger.info('Calling migrate_to_azure_blob (Storage Abstraction)')
                        try:
